...
export default function CameraEntry() {
  ...
  const fetchParking = useCallback(async () => {
    try {
      ...
      if (res.ok) setParking(await res.json());
  ...
  useEffect(() => {
    fetchParking();
    const id = setInterval(fetchParking, 15000);
    return () => clearInterval(id);
  }, [fetchParking]);

  // ── Camera start — tries multiple configs so it works on laptop AND phone ──
  const startCamera = async () => {
    setCameraError(null);

    // Try configs from most specific to most permissive
    const configs = [
    ...
    if (!stream) {
      const msg =
        lastErr?.name === 'NotAllowedError'
          ? 'Camera permission denied. Click the camera icon in the address bar and select Allow, then refresh.'
          : lastErr?.name === 'NotFoundError'
          ? 'No camera detected on this device. Use the Upload option or Manual Entry below.'
          : lastErr?.name === 'NotReadableError'
          ? 'Camera is in use by another app (Zoom, Teams, etc). Close those apps and try again.'
          : `Camera error: ${lastErr?.message ?? 'unknown'}. Use Manual Entry as fallback.`;
      setCameraError(msg);
      toast.error(msg);
      return;
    }
    ...
    if (videoRef.current) {
      ...
      try {
        ...
        setCameraError('Camera stream failed to initialise. Try refreshing the page.');
        return;
      }
    }
  ...
  const handleCapture = async () => {
    ...
    if (!img) { toast.error('Failed to capture frame'); return; }
    stopCamera();
    setCapturedImage(img);
    await processEntry(img);
  };
  ...
  const handleManualEntry = async (e: React.FormEvent) => {
    ...
    try {
      const res  = await fetch(`${API_BASE_URL}/camera-entry/manual`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ plate_number: manualPlate.trim().toUpperCase(), vehicle_color: manualColor || 'unknown' }),
      });
      const data: EntryResult = await res.json();
      setResult(data);
      if (data.success) {
  ...
  const handleExit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) { fetchParking(); setExitPlate(''); }
    } catch {
  ...
  const reset = () => { setCapturedImage(null); setResult(null); stopCamera(); };

  const getColorStyle = (hex: string) => ({ backgroundColor: hex, border: '2px solid rgba(0,0,0,0.15)' });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Auto plate detection → token generation → parking allocation</p>
        </div>
        ...
        <Card className="border-yellow-200 bg-yellow-50">
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setShowManual(false)}><X className="w-4 h-4 text-yellow-600" /></button>
            </div>
            <CardDescription className="text-yellow-700">Use when camera cannot read the plate</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleManualEntry} className="flex flex-col sm:flex-row gap-3">
              <Input placeholder="Plate number e.g. ABC-123-XY" value={manualPlate}
                onChange={e => setManualPlate(e.target.value)} className="flex-1 uppercase" required />
              <Input placeholder="Colour (optional)" value={manualColor}
                onChange={e => setManualColor(e.target.value)} className="w-40" />
          ...
          <CardContent className="space-y-4">
              ...
              {cameraActive && (
                <>
                  ...
                  <Button onClick={handleCapture} disabled={processing}
                    ...
                    <Scan className="w-4 h-4 mr-2" />Capture & Process
                  </Button>
                </>
              )}
              {capturedImage && !cameraActive && (
                <>
                  <Button variant="outline" onClick={reset} className="flex-1">
                    <RefreshCw className="w-4 h-4 mr-2" />New Capture
                  </Button>
                  {!processing && !result && (
                    <Button onClick={() => processEntry(capturedImage)} className="flex-1">
                      <Scan className="w-4 h-4 mr-2" />Re-process
                    </Button>
                  )}
                </>
              )}
            ...
            {!cameraActive && !capturedImage && (
              <div className="border-t border-gray-100 pt-3">
                ...
                <label className="flex items-center gap-2 px-4 py-2 border border-dashed border-gray-300
                  rounded-lg cursor-pointer hover:border-gray-400 hover:bg-gray-50 transition-colors">
          ...
          <CardContent>
            {!result && !processing && (
              <div className="text-center py-16 text-gray-400">
                ...
                <p className="text-sm mt-1 opacity-70">Start the camera and capture a vehicle to begin</p>
              </div>
            )}
            ...
            {result && !processing && (
              <div className="space-y-4">
                <div className={`flex items-center gap-3 p-4 rounded-xl ${
                  ...
                  <div>
                    <p className={`font-semibold ${result.success ? 'text-green-800'
                      : result.registered === false && result.plate_number ? 'text-yellow-800' : 'text-red-800'}`}>
                      {result.success ? 'Entry Granted'
                        : result.registered === false && result.plate_number ? 'Vehicle Not Registered'
                        : 'Entry Denied'}
                    </p>
                    <p className={`text-sm ${result.success ? 'text-green-600'
                ...
                {result.plate_number && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-900 rounded-xl p-4 text-center">
                      ...
                      <p className="text-2xl font-bold font-mono text-white tracking-widest">{result.plate_number}</p>
                      {result.anpr_confidence > 0 && (
                        <p className="text-xs text-gray-400 mt-1">{result.anpr_confidence.toFixed(1)}% confidence</p>
                      )}
                    </div>
                    <div className="bg-gray-50 rounded-xl p-4 text-center border border-gray-200">
                      <p className="text-xs text-gray-500 mb-2">Vehicle Colour</p>
                      <div className="w-12 h-12 rounded-full mx-auto mb-2 shadow-md"
                        style={getColorStyle(result.color_hex || '#808080')} />
                      <p className="font-semibold capitalize text-gray-800">{result.color || 'Unknown'}</p>
                      {result.color_confidence > 0 && (
                        <p className="text-xs text-gray-400">{result.color_confidence.toFixed(1)}% confidence</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                {result.token && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <Badge className="ml-auto bg-blue-100 text-blue-700 hover:bg-blue-100">JWT</Badge>
                    </div>
                    <div className="flex gap-2">
                      <input readOnly value={result.token.token_string}
                    ...
                    <p className="text-xs text-blue-500 mt-2">Expires: {formatExpiry(result.token.expiry_time)}</p>
                  </div>
                )}

                {result.success && (
                  <div className={`rounded-xl p-4 border ${result.parking_slot
                    ? 'bg-purple-50 border-purple-200' : 'bg-orange-50 border-orange-200'}`}>
                    <div className="flex items-center gap-2">
                      <ParkingSquare className={`w-5 h-5 ${result.parking_slot ? 'text-purple-600' : 'text-orange-500'}`} />
                      <span className={`font-semibold text-sm ${result.parking_slot ? 'text-purple-800' : 'text-orange-700'}`}>
                        {result.parking_slot ? 'Parking Slot Assigned' : 'Car Park Full'}
                      </span>
                    </div>
                    {result.parking_slot && (
                      <div className="mt-2 flex items-center gap-4">
                        <span className="text-3xl font-bold text-purple-700">{result.parking_slot}</span>
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={handleExit} className="flex flex-col sm:flex-row gap-3 max-w-lg">
            <Input placeholder="Enter plate number to exit e.g. ABC-123-XY"
              value={exitPlate} onChange={e => setExitPlate(e.target.value.toUpperCase())}
              className="flex-1" required />
            <Button type="submit" disabled={exitLoading} variant="outline"
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Real-time slot occupancy — refreshes every 15 seconds</CardDescription>
            </div>
            {parking && (
              <div className="flex gap-3 text-sm">
                <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full font-medium">{parking.available} Free</span>
                <span className="px-3 py-1 bg-red-100 text-red-700 rounded-full font-medium">{parking.occupied} Occupied</span>
              </div>
            )}
          </div>
        </CardHeader>
          ...
          {parkingLoading ? (
            ...
            <>
              {['A', 'B'].map(zone => (
                <div key={zone} className="mb-6">
                  <h4 className="text-sm font-semibold text-gray-600 mb-3 flex items-center gap-2">
                    <span className="w-6 h-6 bg-slate-700 text-white rounded flex items-center justify-center text-xs">{zone}</span>
                    Zone {zone}
                  </h4>
                  <div className="grid grid-cols-5 sm:grid-cols-10 gap-2">
                    {parking.slots.filter(s => s.zone === zone).map(slot => (
                      <div key={slot.slot_id}
                        title={slot.is_occupied ? `${slot.plate_number} (${slot.vehicle_color ?? 'unknown'})` : 'Available'}
                        className={`relative aspect-square rounded-lg flex flex-col items-center justify-center
                          text-xs font-bold border-2 transition-all cursor-default
                          ${slot.is_occupied ? 'bg-red-100 border-red-300 text-red-700'
                            : 'bg-green-50 border-green-300 text-green-700 hover:bg-green-100'}`}>
                        <span className="text-[10px] font-mono">{slot.slot_id.replace('SLOT-', '')}</span>
                        {slot.is_occupied && slot.vehicle_color && slot.vehicle_color !== 'unknown' && (
                          <div className="w-3 h-3 rounded-full mt-0.5 border border-white/50"
              ...
              <div className="flex items-center gap-6 mt-2 text-xs text-gray-500 border-t border-gray-100 pt-3">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded-full bg-gray-400 inline-block" />Colour dot = vehicle colour
                </span>
              </div>
            </>
          )}
